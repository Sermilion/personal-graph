...
private const val MAX_RELEVANCE_BOOST: Int = 100
...
internal fun CompactMapEntry.suggestionScore(domain: RetrievalDomain): Int = when {
  ...
  type == TYPE_PATTERN -> SCORE_PATTERN
  isState() -> SCORE_STATE
  else -> SCORE_DEFAULT
}

internal fun CompactMapEntry.suggestionScore(
  domain: RetrievalDomain,
  relevanceTerms: Set<String>,
): Int = suggestionScore(domain) + relevanceBoost(relevanceTerms)
...
internal fun CompactMapEntry.mapBudgetScore(domain: RetrievalDomain): Int = when {
  ...
  globalPreference() -> SCORE_GLOBAL_PREFERENCE
  type == TYPE_PATTERN -> SCORE_MAP_PATTERN
  type == "subject" -> SCORE_MAP_SUBJECT
  isState() -> SCORE_MAP_STATE
  else -> SCORE_DEFAULT
}

internal fun CompactMapEntry.mapBudgetScore(
  domain: RetrievalDomain,
  relevanceTerms: Set<String>,
): Int = mapBudgetScore(domain) + relevanceBoost(relevanceTerms)
...
private fun CompactMapEntry.domainEntry(domain: RetrievalDomain) = domain != RetrievalDomain.General &&
...
private fun CompactMapEntry.scopedStateFor(domain: RetrievalDomain) = isState() &&
  domain != RetrievalDomain.General &&
  scopedTo(domain)

private fun CompactMapEntry.generalGlobalState(domain: RetrievalDomain) = domain == RetrievalDomain.General &&
  isState() &&
  unscoped()
...
private fun CompactMapEntry.relevanceBoost(relevanceTerms: Set<String>): Int = relevanceBoost(
  relevanceTerms,
  MAX_RELEVANCE_BOOST,
)
