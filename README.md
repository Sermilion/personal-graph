...
Install the MCP server for local agents:

```bash
scripts/install-local-mcp --vault /absolute/path/to/your/vault
```

The installer builds the MCP distribution, copies it to `~/.local/share/personal-graph/mcp-server/`, writes the stable launcher `~/.local/bin/personal-graph-mcp`, and updates known local agent MCP configs to use that launcher. Re-run it after source changes that should be reflected in local agents.
...
```bash
~/.local/bin/personal-graph-mcp --vault /absolute/path/to/your/vault
```
